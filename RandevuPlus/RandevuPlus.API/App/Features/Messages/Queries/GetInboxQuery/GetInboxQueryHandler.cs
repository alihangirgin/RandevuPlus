...
{
    public class GetInboxQueryHandler : IRequestHandler<GetInboxQuery, Result<List<GetInboxQueryResponseItem>>>
    {
        ...
        public async Task<Result<List<GetInboxQueryResponseItem>>> Handle(GetInboxQuery query, CancellationToken cancellationToken)
        {
            var userId = _currentUserService.UserId.Value;
            var inboxMessages = await _unitOfWork.Messages.GetPaginatedAsync(query.PageNumber, query.PageSize,
                filter: x => x.ReceiverId == userId && !x.IsRead && !x.IsRemovedFromReceiver, orderBy: x => x.OrderBy(y => y.CreatedAt));
